...
const RregisterPage = () => {
    ...
    const handleRegister = (e) => {
        ...
        if (!uppercasePattern.test(password)) {
            toast.error('Must include at least 1 uppercase letter');
            return;
        }
        if (!lowercasePattern.test(password)) {
            toast.error('Must include at least 1 lowercase letter');
            return;
        }
            ...
            <div className='py-[65px] lg:py-[95px]'>
                <div className='container'>
                    <div className='max-w-[510px] mx-auto px-[25px] py-7 md:py-10 md:px-10 bg-white shadow-xl rounded-md border border-[#e7e7e798]'>
                        ...
                        <form onSubmit={handleRegister}>
                            <fieldset className="fieldset">
                                ...
                                <div className='relative'>
                                    ...
                                    <span onClick={() => setShowPass(!showPass)} className='absolute top-1/2 -translate-y-1/2 right-5 cursor-pointer text-[#141414]'>{showPass ? <EyeOff size={20} /> : <Eye size={20} />}</span>
                                </div>
                                {/* Button */}
                                <button type="submit" className='py-2 w-full bg-[#219E64] rounded mt-5 text-white text-[18px] font-medium cursor-pointer'>Register</button>
                            </fieldset>
                        </form>
                        ...
                        <p className='text-center text-[#464545] text-[16px] font-medium pt-[22px]'>Already have an account? <Link href="/login" className='text-[#219E64] font-semibold' to='/login'>Login</Link></p>
                    </div>
                </div>
            </div>
        </>
